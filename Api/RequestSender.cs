...
namespace ShipGameFront
{
    class RequestSender
    {
        public static async Task<ServiceResponse> Get(string path, string queryString)
        {
            ...
            var builder = new UriBuilder("http://localhost:5243" + path + queryString);
            ...
            response.StatusCode = (int)responseTask.StatusCode;
        ...
        public static async Task<ServiceResponse> Post(string path, string content)
        {
            ServiceResponse response = new ServiceResponse();
            var client = new HttpClient();
            var uri = new Uri("http://localhost:5243" + path);
